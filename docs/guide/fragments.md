...
- **Safe refactors.** Pulling a section into a fragment is a syntactic move — if the [tests](/guide/testing) stay green, the behaviour is identical.
...
The way to be sure is the [test suite](/guide/testing). A tree with scenarios for its main paths means:
...
- [Writing trees](/guide/writing-trees) — the YAML shape every fragment shares with a root tree.
