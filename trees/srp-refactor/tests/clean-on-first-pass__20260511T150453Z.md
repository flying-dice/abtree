...
**Target execution:** clean-refactor-on-first-pass-test__abtree-example-tree__1
**Overall:** PASS
...
| initial_violations | [{"path":"tests/fixtures/handler.ts","severity":"critical","responsibilities":[" |
...
| chosen_violation | tests/fixtures/handler.ts: handleOrder mixes routing, validation, pricing, persi |
...
|---|---|---|---|
| status | done | done | ✓ |
| local.chosen_violation | starts with tests/fixtures/handler.ts | tests/fixtures/handler.ts: handleOrder m | ✓ |
| local.refactor_complete | true | true | ✓ |
| local.refactor_summary | non-empty | Split tests/fixtures/handler.ts into sin | ✓ |
| local.has_critical_violations | false | false | ✓ |
| local.review_report | non-empty | No high-signal issues found in the handl | ✓ |
...
```mermaid
---
title: "Clean refactor on first pass test (complete)"
---
flowchart TD
    Top{{"Top\n[sequence]"}}
