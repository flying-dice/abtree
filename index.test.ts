...
function abt(args: string[], cwd: string): { stdout: string; stderr: string; exitCode: number } {
...
test("hello-world flow: full execution reaches done", () => {
  // Create flow
  const createOut = abt(["flow", "create", "hello-world", "integration test"], tmp);
  ...
  expect(step.type).toBe("instruct");
  expect(step.name).toBe("Compose_Response");
  localWrite("response", "Good morning, test-user! Weather unavailable. No news today.");
  submit("success");
