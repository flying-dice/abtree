# Test report — Cycle completes but coupling regressed below threshold → Cycle_Partial wins

**Tree:** improve-codebase (v1.1.0)
**Runner:** test-tree (v1.2.0, fixture-driven side effects)
**Spec:** .abtree/trees/improve-codebase/TEST__partial-verdict.yaml
**Target execution:** test-partial-verdict-coupling-regression__improve-codebase__1
**Overall:** PASS
...
| baseline_scores | { dry: 0.52, srp: 0.61, coupling: 0.72, cohesion: 0.71 } |
| refactor_queue | [] |
| done_log | [dry-1 @ 0.84, srp-1 @ 0.82] |
| failed_log | [] |
| stage_halt | false |
| final_scores | { dry: 0.84, srp: 0.82, coupling: 0.66, cohesion: 0.71 } |
...
| local.final_scores | non-empty | non-empty (coupling 0.66 < 0.7) | ✓ |
| runtime.retry_count.Refactor_Item | 0 | 0 | ✓ |

**Note:** Cycle_Passed evaluated false because `final_scores.coupling = 0.66` fell below the 0.7 threshold despite DRY and SRP both clearing comfortably. The selector fell through to Cycle_Partial, which surfaced a partial-improvement report covering deltas, done_log, and failed_log — exactly the post-condition the spec calls for.

## Trace

```mermaid
---
title: "test-partial-verdict coupling regression (complete)"
---
flowchart TD
    ...
    style 0_11_Cycle_Passed fill:#f87171,stroke:#dc2626,color:#450a0a
    0_11_Cycle_Partial["Cycle Partial\n[action]"]
    0_Cycle_Verdict --> 0_11_Cycle_Partial
    style 0_11_Cycle_Partial fill:#4ade80,stroke:#16a34a,color:#052e16
```
