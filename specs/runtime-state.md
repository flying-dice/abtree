...
title: Runtime State and Flow Document Anatomy
status: shipped
author: Starscream
created: 2026-05-09
...
- `global` is read-only via the CLI (`abtree global read`). Set once at flow creation from the tree's `state.global` block.
...
- A flow ID matches `^[a-z0-9_-]+__[a-z0-9_-]+__\d+$`. Anything else fails before any file open.
- Reads of legacy flow documents (predating the `runtime` field) lazily migrate `_node_status__*` and `_step__*` keys out of `local` into `runtime`. The migration is idempotent.
...
- Corrupt JSON throws `Corrupt flow file: ${id}`. `list()` skips corrupt files silently with stderr noise.

### CLI surface

| Command | Touches |
|---|---|
| `abtree local read <flow> [path]` | `doc.local` only |
| `abtree local write <flow> <path> <value>` | `doc.local` only |
| `abtree global read <flow> [path]` | `doc.global` only |
| `abtree flow get <flow>` | Returns the entire document (including `runtime`) — useful for debugging. |
| `abtree next` / `eval` / `submit` | Mutates `runtime`, sets `cursor` / `phase` / `status`. |

The asymmetry between `local read` (scope-locked) and `flow get` (full document) is deliberate: `flow get` is a dump for inspection; `local read` is the operational interface the agent uses inside a flow.

### Retry counter persistence

When a node with `retries: N` fails, the runtime increments `runtime.retry_count[path]` and wipes `runtime.node_status` and `runtime.step_index` keys whose path is `path` itself or a descendant. Retry counts are NOT wiped during the reset — they accumulate across the lifetime of the flow.

### Lazy migration

Flow documents created before the `runtime` field existed have `_node_status__*` and `_step__*` keys mixed in with user data in `local`. On every `readDoc(id)`, if `runtime` is undefined, `migrateRuntime` walks `local`, lifts keys with those prefixes into the new structure, normalises the path encoding (underscores → dots), and writes the result. Idempotent: `runtime` defined → no-op.

## Affected Systems

- `src/types.ts` — `FlowDoc`, `RuntimeState`, `NodeStatus`.
- `src/repos.ts` — `FlowStore` (lazy `migrateRuntime`, `getRuntime*` / `setRuntime*` helpers, `resetRuntimeSubtree`, `incrementRuntimeRetryCount`).
...
- Driving any flow to completion: `abtree local read` returns user-written keys only, no `_node_status__*` or `_step__*` leaked.
- `abtree flow get` returns the full document including the `runtime` field.
- A retry-driven flow (e.g. `counter-demo`) shows `runtime.retry_count` populated; `local` shows only user data.
- Loading a legacy flow that has `_node_status__*` keys in `local`: the next read returns `local` without those keys and `runtime.node_status` populated.
- A killed mid-flow can be resumed: cursor / phase / runtime state are all on disk; `abtree next <flow>` picks up where it left off.
